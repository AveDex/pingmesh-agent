...
# Build
RUN CGO_ENABLED=0 GOOS=${OS} GOARCH=${ARCH} go build -a -mod=vendor -o pingmesh-agent .

FROM quay.io/prometheus/busybox-${OS}-${ARCH}:latest

COPY --from=builder /workspace/pingmesh-agent  /bin/pingmesh-agent
COPY pingmesh.yml   /etc/pingmesh-agent/pingmesh.yml
COPY pinglist.yml   /etc/pingmesh-agent/pinglist.yml

EXPOSE      9115
ENTRYPOINT  [ "/bin/pingmesh-agent" ]
CMD         [ "--config.file=/etc/pingmesh-agent/pingmesh.yml", "--pinglist.file=/etc/pingmesh-agent/pinglist.yml" ]
